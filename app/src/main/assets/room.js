...
(function () {
  ...
  window.__museroom = {
    ...
    /*
     * Get a track ready without letting a note of it out.
     *
     * A room now starts every song at an agreed moment rather than whenever
     * each phone finished fetching, so the fetching has to happen beforehand
     * and stay silent when it lands. cueVideoById is the player's own word for
     * exactly this and is tried first; where it is missing, loading and
     * stopping immediately gets to the same place, and the volume is taken
     * down across the gap so that nothing escapes if the pause lands a frame
     * late.
     */
    preload: function (id, startSeconds) {
      wanted = id || '';
      lastError = '';
      var p = player();
      if (!p) return false;
    ...
    /*
     * The agreed moment. Put exactly where everybody else is being put, then
     * let go, with the sound turned back up as late as possible.
     */
